<?php
  ...
  if (!isset($_SESSION['user_id'])) {
    ...
    exit();
  	...
  	echo ('<p class="login">You are logged in as ' . $_SESSION['username'].'.<a href="logout.php"> logout.php</a>.</p>');
  }
  ...
  if (mysqli_num_rows($data) == 1) {
    ...
    if (!empty($row['picture'])) {
      echo '<tr><td class="label">Picture:</td><td><img src="' . MM_IMAGES . $row['picture'] .
